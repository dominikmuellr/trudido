...
class FilesChannel {
  ...
  Future<void> ensureInitialized() async {
    ...
    if (!Platform.isAndroid) { _initialized = true; return; }
    _ch.setMethodCallHandler((call) async {
      if (call.method == 'onImport') {
        final jsonStr = call.arguments as String?;
        debugPrint('[FilesChannel] Received import data, length: ${jsonStr?.length ?? 0}');
        if (jsonStr == null) {
        ...
        try {
          ...
          debugPrint('[FilesChannel] JSON parsed successfully, keys: ${map.keys.toList()}');
