...
class NavigationService {
  ...
  static Future<T?> navigateTo<T extends Object?>(Route<T> route) {
    ...
    if (navigator == null) {
      throw Exception('Navigator not available. Make sure NavigationService.navigatorKey is assigned to MaterialApp.navigatorKey');
    }
    return navigator.push(route);
  }
  
  /// Navigate to a new route and replace the current one
  static Future<T?> navigateAndReplace<T extends Object?, TO extends Object?>(Route<T> newRoute) {
    final navigator = NavigationService.navigator;
    if (navigator == null) {
      throw Exception('Navigator not available. Make sure NavigationService.navigatorKey is assigned to MaterialApp.navigatorKey');
    }
  ...
  static Future<T?> navigateToNamed<T extends Object?>(String routeName, {Object? arguments}) {
    final navigator = NavigationService.navigator;
    if (navigator == null) {
      throw Exception('Navigator not available. Make sure NavigationService.navigatorKey is assigned to MaterialApp.navigatorKey');
    }
