...
class MarkdownExportService {
  ...
  static Future<bool> exportNotesToFiles() async {
    try {
      ...
      if (selectedDirectory == null) {
        debugPrint('[MarkdownExport] User cancelled directory selection');
        return false;
      }
      ...
      debugPrint('[MarkdownExport] Exported $exportedCount/${notes.length} notes to ${notesDir.path}');
  ...
  static Future<bool> _exportSingleNote(Note note, String directoryPath) async {
    try {
      ...
      debugPrint('[MarkdownExport] Exported note: ${note.title} -> $fileName.md');
      return true;
  ...
  static String _generateSafeFileName(Note note) {
    ...
    final datePrefix = note.createdAt.toIso8601String().substring(0, 10); // YYYY-MM-DD
  ...
  static Future<ImportResult> _importFromMobileFilePicker() async {
    try {
      ...
      debugPrint('[MarkdownImport] Mobile import completed: $importedCount imported, $skippedCount skipped, ${errors.length} errors');
      
      if (importedCount > 0) {
        final message = 'Successfully imported $importedCount note(s)${skippedCount > 0 ? ' ($skippedCount skipped as duplicates)' : ''}${errors.isNotEmpty ? '. ${errors.length} files had errors.' : ''}';
        return ImportResult(success: true, message: message);
      } else {
        return ImportResult(
          success: false, 
          message: 'No notes imported.${errors.isNotEmpty ? " Errors: ${errors.take(3).join(", ")}" : ""}'
        );
      }
  ...
  static Future<ImportResult> _importFromDesktopDirectory() async {
    try {
      ...
      String? selectedDirectory = await FilePicker.platform.getDirectoryPath();
      if (selectedDirectory == null) {
        debugPrint('[MarkdownImport] User cancelled directory selection');
        return ImportResult(success: false, message: 'Import cancelled');
      }
      ...
      debugPrint('[MarkdownImport] Desktop import completed: $importedCount imported, $skippedCount skipped, ${errors.length} errors');
      ...
      if (importedCount > 0) {
        ...
        return ImportResult(
          success: false, 
          message: 'No notes imported.${errors.isNotEmpty ? " Errors: ${errors.join(", ")}" : ""}'
        );
      }
  ...
  static Future<ImportResult> _importSingleFile(File file) async {
    try {
      ...
      if (note == null) {
        return ImportResult(success: false, message: 'Could not parse markdown file');
      }
      ...
      final existingById = existingNotes.where((n) => n.id == note.id).firstOrNull;
      if (existingById != null) {
        debugPrint('[MarkdownImport] Skipping duplicate note (ID exists): ${note.title}');
        return ImportResult(success: false, message: 'skipped');
      }
      
      // Check for existing note by title and content similarity
      final similarNote = existingNotes.where((n) => 
        n.title == note.title && 
  ...
  static Note? _parseMarkdownFile(String content, String filePath) {
    try {
      ...
      if (title.isEmpty) {
        final h1Match = RegExp(r'^# (.+)$', multiLine: true).firstMatch(noteContent);
        if (h1Match != null) {
      ...
      if (title.isEmpty) {
        ...
        title = fileName.replaceAll('.md', '').replaceAll(RegExp(r'^\d{4}-\d{2}-\d{2}-'), '');
