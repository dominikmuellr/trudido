...
class UnifiedSettingsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<UnifiedSettingsPage> createState() => _UnifiedSettingsPageState();
}

class _UnifiedSettingsPageState extends ConsumerState<UnifiedSettingsPage> with WidgetsBindingObserver {
  @override
  void initState() {
    super.initState();
  ...
  return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            const Text('Control permissions & system settings that affect reminder timing.',
              style: TextStyle(fontSize: 14)),
            const SizedBox(height: 16),
            _StatusTile(
              title: 'Notifications',
              status: notifEnabledAsync.maybeWhen(orElse: () => true, data: (v) => v),
                ...
                if (stillDisabled && context.mounted) {
                  final open = await showDialog<bool>(
                    ...
                    builder: (c) => AlertDialog(
                      ...
                      content: const Text('Permission not granted. Open system notification settings?'),
                      actions: [
        ...
        onTap: () async {
          ...
          final localCtx = context;
                  ...
                  if (proceed) {
                    ...
                    await SystemSettingsService.instance.openExactAlarmSettings();
                  }
        ...
        onTap: () async {
          ...
          final localCtx = context;
                  ...
                  final proceed = await _showRationale(localCtx,
                      ...
                      action: 'Open Settings');
                  if (proceed) {
                    await SystemSettingsService.instance.requestIgnoreBatteryOptimizations();
                  }
                  await Future.delayed(const Duration(milliseconds: 350));
                  await watcher.refresh();
            ...
            const Text('• Exact alarms show no popup: you must toggle in system settings.\n'
            ...
            if (const bool.fromEnvironment('dart.vm.product') == false) ...[
              ...
              const SizedBox(height: 8),
              const Text('Use this once to force system to list the app under "Alarms & reminders".'
  ...
  static Future<bool> _showRationale(BuildContext context, {required String title, required String body, required String action}) async {
...
class _StatusTile extends StatelessWidget {
  final String title; final bool status; final String description; final VoidCallback onTap; final bool loading;
  const _StatusTile({required this.title, required this.status, required this.description, required this.onTap, this.loading=false});
  @override
  Widget build(BuildContext context) {
    final color = loading
    ...
    return Card(
      child: ListTile(
        leading: icon,
        title: Text(title),
        subtitle: Text('$description\nStatus: ${loading ? '…' : status ? 'Enabled' : 'Disabled'}'),
        isThreeLine: true,
