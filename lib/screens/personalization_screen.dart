      ...
      body: ListView(
        children: [
          ...
          Consumer(
            builder: (context, ref, _) {
              ...
              return ListTile(
                ...
                subtitle: Text(_getEditorFontDisplayName(editorFont)),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => _showEditorFontPicker(context, ref),
              );
            },
          ),
          Consumer(
            builder: (context, ref, _) {
              final fontSize =
                  ref.watch(preferencesStateProvider).editorFontSize;
              return ListTile(
                leading: const Icon(Icons.format_size),
                title: const Text('Editor Font Size'),
                subtitle: Text('${fontSize.toStringAsFixed(0)} pt'),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => _showEditorFontSizePicker(context, ref),
              );
            },
          ),
          Consumer(
          ...
          Consumer(
            builder: (context, ref, _) {
              final spacing =
                  ref.watch(preferencesStateProvider).paragraphSpacing;
              return ListTile(
                leading: const Icon(Icons.space_bar_outlined),
                title: const Text('Paragraph Spacing'),
                subtitle: Text(
                  spacing == 0 ? 'None' : '${spacing.toStringAsFixed(0)} pt',
                ),
                trailing: const Icon(Icons.chevron_right),
...
class _ThemeModeSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isMonochromeTheme = accentColorSeed == 0xFF9E9E9E && !useDynamicColor;
...
String _getEditorFontDisplayName(String fontFamily) {
  switch (fontFamily) {
    case 'default':
...
void _showEditorFontPicker(BuildContext context, WidgetRef ref) {
  final controller = ref.read(preferencesControllerProvider);
  ...
  showModalBottomSheet(
    ...
    builder: (context) => ListView(
      shrinkWrap: true,
      children: [
        _buildEditorFontOption(
          context, controller, currentFont,
          'default', 'Default (app font)', 'Same as the global app font',
          Icons.font_download_outlined,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
          'inter', 'Inter', 'Modern, geometric UI typeface',
          Icons.text_fields,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
          'roboto', 'Roboto', 'Material default',
          Icons.android,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
          'lexend', 'Lexend', 'Enhanced readability',
          Icons.visibility,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
          'opensans', 'Open Sans', 'Clean & neutral',
          Icons.font_download,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
          'jetbrains', 'JetBrains Mono', 'Code & technical',
          Icons.code,
        ),
        _buildEditorFontOption(
          context, controller, currentFont,
...
void _showEditorFontSizePicker(BuildContext context, WidgetRef ref) {
  final controller = ref.read(preferencesControllerProvider);
      ...
      children: [
        ...options.map(
          (v) => RadioListTile<double>(
            value: v,
            groupValue: current,
            title: Text(v == 0 ? 'None (0 pt)' : '${v.toStringAsFixed(0)} pt'),
            // ignore: deprecated_member_use
            onChanged: (picked) {
              if (picked != null) {
