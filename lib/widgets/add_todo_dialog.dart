...
class _AddTodoDialogState extends ConsumerState<AddTodoDialog> {
                      ...
                      trailing: _dueDate != null 
                          ? IconButton(
                              ...
                              onPressed: () => setState(() {
                                _dueDate = null;
                                _dueTime = null;
                                _reminderOffsetsMinutes.clear(); // Clear reminders when due date is removed
                              }),
                            ) 
                          : null,
  ...
  debugPrint('AddTodoDialog: _selectDueDate() called');
  ...
  debugPrint('AddTodoDialog: Date picker result: $date');
    if (date != null) {
      setState(() {
        _dueDate = date;
        // Add default reminder if no reminders exist
        if (_reminderOffsetsMinutes.isEmpty) {
          debugPrint('AddTodoDialog: Adding default reminder at due time');
          _reminderOffsetsMinutes.add(0); // At due time
        }
  debugPrint('AddTodoDialog: Due date set, current reminders: $_reminderOffsetsMinutes');
      });
    }
  }
