...
/** Stores minimal task status flags for idempotent native actions. */
object TaskStatusStore {
    private const val PREFS = "task_status_store"
    private const val COMPLETED_SET = "completed_set" // pipe-separated list

    private fun prefs(ctx: Context) = ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE)

    fun isCompleted(ctx: Context, taskId: String): Boolean {
        val raw = prefs(ctx).getString(COMPLETED_SET, "") ?: ""
        return raw.split('|').contains(taskId)
    }
